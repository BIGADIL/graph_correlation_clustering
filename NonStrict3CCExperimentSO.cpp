...
int main(int argc, char *argv[]) {
  ...
  std::filesystem::create_directory("non_strict_3cc_so");
  auto path = std::filesystem::current_path();
  path.append("non_strict_3cc_so");
  ...
  for (const auto &graph_size: ep.GetGraphSize()) {
    for (const auto &density: ep.GetDensity()) {
      StackOverflowGraphFactory graphs_factory(R"(D:\run\graph_correlation_clustering\Tags_5.json)", density);
      const auto dir_name = "n-" + std::to_string(graph_size) + "-noise-" + std::to_string(density) + "/";
      ...
      for (unsigned i = 0; i < ep.GetNumGraphs(); ++i) {
        ...
        name << dir_name << "n-" << graph_size << "-noise-" << density << "-" << dis_(gen_) << ".json";
        out.open(name.str());
        out << report;
        out.close();
      }

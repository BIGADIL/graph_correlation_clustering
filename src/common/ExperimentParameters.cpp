...
ExperimentParameters::ExperimentParameters(std::vector<unsigned> graph_size_vector, std::vector<double> density_vector,
                                           std::vector<std::string> distribution_vector,
                                           std::vector<std::string> algorithms_vector, std::vector<double> parts,
                                           const unsigned num_threads, const unsigned num_graphs)
...
ExperimentParameters ExperimentParameters::ReadFromConfig(const std::string& path) {
  ...
  return {graph_size_vector,          density_vector, distribution_vector,
          algorithms_vector,          parts,          doc["num_threads"].GetUint(),
          doc["num_graphs"].GetUint()};
}

unsigned int ExperimentParameters::GetNumThreads() const { return num_threads_; }

unsigned int ExperimentParameters::GetNumGraphs() const { return num_graphs_; }

const std::vector<unsigned int>& ExperimentParameters::GetGraphSize() const { return graph_size_vector_; }

const std::vector<double>& ExperimentParameters::GetDensity() const { return density_vector_; }

const std::vector<std::string>& ExperimentParameters::GetAlgorithms() const { return algorithms_vector_; }

const std::vector<double>& ExperimentParameters::GetParts() const { return parts_; }

const std::vector<std::string>& ExperimentParameters::GetDistribution() const { return distribution_vector_; }

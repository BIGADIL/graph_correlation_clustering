...
void non_strict_3cc::TwoVerticesNeighborhoodWithLocalSearch::
    BestNeighborhoodClusteringThreadWorker(
        const IGraph &graph, const unsigned thread_id,
        std::vector<Solution> &local_thread_buffer) const {
  for (unsigned i = thread_id; i < graph.Size(); i += num_threads_) {
    for (unsigned j = 0; j < graph.Size(); ++j) {
      if (i == j) continue;
      auto tmp_clustering =
          neighbor_splitter_.SplitGraphByTwoVertices(graph, i, j);
...
non_strict_3cc::TwoVerticesNeighborhoodWithLocalSearch::
...
IClustPtr non_strict_3cc::TwoVerticesNeighborhoodWithLocalSearch::
...
std::vector<Solution>
non_strict_3cc::TwoVerticesNeighborhoodWithLocalSearch::getAllSolutions(
  ...
  for (unsigned i = 0; i < num_threads_; i++) {
    thread_vector[i] =
        std::thread(&TwoVerticesNeighborhoodWithLocalSearch::
                        BestNeighborhoodClusteringThreadWorker,
  ...
  auto lo =
      LocalSearch::ComputeLocalOptimum(graph, best_neighborhood_clustering);

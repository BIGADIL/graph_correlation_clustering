...
void non_strict_3cc::NeighborhoodWithLocalSearch::BestNeighborhoodClusteringThreadWorker(
    const IGraph& graph, const unsigned thread_id, IClustPtr& local_best_clustering) const {
  ...
  for (unsigned i = thread_id; i < graph.Size(); i += num_threads_) {
    ...
    for (unsigned j = 0; j < graph.Size(); ++j) {
      ...
      tmp_clustering = LocalSearch::ComputeLocalOptimum(graph, tmp_clustering, SECOND_CLUSTER, THIRD_CLUSTER);
...
non_strict_3cc::NeighborhoodWithLocalSearch::NeighborhoodWithLocalSearch(const unsigned num_threads,
...
IClustPtr non_strict_3cc::NeighborhoodWithLocalSearch::getBestNeighborhoodClustering(const IGraph& graph) const {
  ...
  for (unsigned i = 0; i < num_threads_; i++) {
    thread_vector[i] = std::thread(&NeighborhoodWithLocalSearch::BestNeighborhoodClusteringThreadWorker, this,

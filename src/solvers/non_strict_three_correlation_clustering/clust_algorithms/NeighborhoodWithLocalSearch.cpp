...
void non_strict_3cc::NeighborhoodWithLocalSearch::BestNeighborhoodClusteringThreadWorker(const IGraph &graph,
  ...
  for (unsigned i = threadId; i < graph.Size(); i += num_threads_) {
    auto tmp_clustering = neighbor_splitter_.BuildFirstCluster(graph, i);
    for (unsigned j = 0; j < graph.Size(); ++j) {
      if (!tmp_clustering->IsNonClustered(j)) continue;
      tmp_clustering = neighbor_splitter_.BuildSecondAndThirdClusters(graph, tmp_clustering, j);

...
void non_strict_3cc::NeighborhoodWithLocalSearch::
  ...
  for (unsigned i = thread_id; i < graph.Size(); i += num_threads_) {
    ...
    for (unsigned j = 0; j < graph.Size(); ++j) {
      ...
      auto tmp_clustering = NeighborSplitter::BuildSecondAndThirdClusters(
          graph, part_clustering, j);
      tmp_clustering = LocalSearch::ComputeLocalOptimum(
          graph, tmp_clustering, SECOND_CLUSTER, THIRD_CLUSTER);
      if (const auto tmp_distance = tmp_clustering->GetDistanceToGraph(graph);
...
non_strict_3cc::NeighborhoodWithLocalSearch::NeighborhoodWithLocalSearch(
...
IClustPtr
non_strict_3cc::NeighborhoodWithLocalSearch::getBestNeighborhoodClustering(

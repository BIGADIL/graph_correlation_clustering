...
non_strict_3cc::NonStrict3CCSolver::NonStrict3CCSolver(
    const unsigned int num_threads, IClustFactoryPtr factory)
    : num_threads_(num_threads), factory_(std::move(factory)) {}

std::string non_strict_3cc::NonStrict3CCSolver::solve(
  ...
  if (std::ranges::find(used_algorithms,
    ...
    infos.emplace_back("TwoVerticesNeighborhoodWithLocalSearch", clustering,
                       clustering->GetDistanceToGraph(*graph),
                       std::chrono::duration_cast<std::chrono::seconds>(
                           std::chrono::steady_clock::now() - start_time));
  }
  if (std::ranges::find(used_algorithms,
    ...
    auto start_time = std::chrono::steady_clock::now();
    auto clustering = twnls.getBestNeighborhoodClustering(*graph);
    infos.emplace_back("TwoVerticesNeighborhoodWithManyLocalSearches",
                       clustering, clustering->GetDistanceToGraph(*graph),
                       std::chrono::duration_cast<std::chrono::seconds>(
                           std::chrono::steady_clock::now() - start_time));
  }
  if (std::ranges::find(used_algorithms, "Genetic") != used_algorithms.end()) {
    IPLSAlgorithm genetic(100, 6, factory_, 32, 5, 0.4, num_threads_);
    auto start_time = std::chrono::steady_clock::now();

...
__host__ __device__ inline void PairFromIndex(const unsigned long long q,
                                              const unsigned n,
                                              unsigned &first,
                                              unsigned &second) {
 ...
 */
__global__ void SplitPairsKernel(const uint8_t *__restrict__ adj,
...
__global__ void TwoVerticesKernel(const uint8_t *__restrict__ adj,
                                  ...
                                  Label *__restrict__ inout_labels,
  ...
  int *gains = kLocalSearch ? gains_scratch + static_cast<size_t>(blockIdx.x) *
                                                  n * kNumLabels
                            : nullptr;
  unsigned distance = non_strict_3cc::cuda::GainsFromGemm(
      n, s_labels, g, n_pad, num_edges, gains);
...
non_strict_3cc::CudaTwoVerticesNeighborhood::Run(
  ...
  const size_t per_pair =
      static_cast<size_t>(n_pad) * kNumLabels * (sizeof(int8_t) + sizeof(int)) +
  ...
  const unsigned long long num_edges =
      std::accumulate(flat.begin(), flat.end(), 0ull) / 2;
  ...
  for (unsigned long long first = 0; first < num_pairs; first += chunk) {
    const unsigned count = static_cast<unsigned>(
        std::min<unsigned long long>(chunk, num_pairs - first));
    SplitPairsKernel<<<count, threads, labels_bytes>>>(
        d_adj.Get(), n, n_pad, first, d_labels.Get(), d_lmat.Get());
    GCC_CUDA_CHECK(cudaGetLastError());
    gcc_cuda::GemmInt8(cublas, n_pad, count * kNumLabels, n_pad, d_amat.Get(),
                       d_lmat.Get(), d_gmat.Get());
    if (with_local_search) {
      TwoVerticesKernel<true><<<count, threads, labels_bytes>>>(
          d_adj.Get(), n, n_pad, num_edges, d_gmat.Get(), d_gains.Get(),
          d_labels.Get(), d_dist.Get());
    } else {
      TwoVerticesKernel<false><<<count, threads, labels_bytes>>>(
          d_adj.Get(), n, n_pad, num_edges, d_gmat.Get(), nullptr,
          d_labels.Get(), d_dist.Get());
    }
    GCC_CUDA_CHECK(cudaGetLastError());

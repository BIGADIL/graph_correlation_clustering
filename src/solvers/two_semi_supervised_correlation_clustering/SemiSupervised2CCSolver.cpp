...
std::string semi_supervised_2cc::SemiSupervised2CCSolver::solve(const IGraphPtr& graph, const double density,
  ...
  if (std::ranges::find(used_algorithms, "NeighborhoodWithManyLocalSearches") != used_algorithms.end()) {
    ...
    auto clustering = nmls.getBestNeighborhoodClustering(*graph, first_cluster_vertex, second_cluster_vertex);
  ...
  if (std::ranges::find(used_algorithms, "NeighborhoodWithOneLocalSearch") != used_algorithms.end()) {
    ...
    auto start_time = std::chrono::steady_clock::now();
    auto clustering = nols.getBestNeighborhoodClustering(*graph, first_cluster_vertex, second_cluster_vertex);
  ...
  if (std::ranges::find(used_algorithms, "Neighborhood") != used_algorithms.end()) {
    ...
    auto start_time = std::chrono::steady_clock::now();
    auto clustering = n.getBestNeighborhoodClustering(*graph, first_cluster_vertex, second_cluster_vertex);
  ...
  if (std::ranges::find(used_algorithms, "NeighborhoodOfPreClusteringVertices") != used_algorithms.end()) {
    ...
    auto start_time = std::chrono::steady_clock::now();
    auto clustering = npcv.getBestNeighborhoodClustering(*graph, first_cluster_vertex, second_cluster_vertex);
  ...
  if (std::ranges::find(used_algorithms, "BranchAndBounds") != used_algorithms.end()) {
    ...
    auto approximate_clustering =
        nmls.getBestNeighborhoodClustering(*graph, first_cluster_vertex, second_cluster_vertex);
    BranchAndBounds bb;
    auto start_time = std::chrono::steady_clock::now();
    auto clustering = bb.GetBestClustering(graph, approximate_clustering, first_cluster_vertex, second_cluster_vertex);
  ...
  return FormatComputationToJson(*graph, infos, graph->Size(), density, first_cluster_vertex, second_cluster_vertex);
}
...
std::string semi_supervised_2cc::SemiSupervised2CCSolver::FormatComputationToJson(
    const IGraph& graph, const std::vector<ClusteringInfo>& computation_results, const unsigned int size,
    const double density, const unsigned first_cluster_vertex, const unsigned second_cluster_vertex) {
  std::stringstream ss;
  ss << "{ " << std::endl;

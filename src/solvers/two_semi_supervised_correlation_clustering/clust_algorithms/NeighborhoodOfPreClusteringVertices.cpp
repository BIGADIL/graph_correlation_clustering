...
semi_supervised_2cc::NeighborhoodOfPreClusteringVertices::
    NeighborhoodOfPreClusteringVertices(IClustFactoryPtr clustering_factory)
    : clustering_factory_(std::move(clustering_factory)) {}

IClustPtr semi_supervised_2cc::NeighborhoodOfPreClusteringVertices::
    getBestNeighborhoodClustering(
        const IGraph &graph, const unsigned int first_cluster_vertex,
  ...
  for (unsigned i = 0; i < graph.Size(); ++i) {
    if ((i == first_cluster_vertex ||
         graph.IsJoined(i, first_cluster_vertex)) &&
        i != second_cluster_vertex) {
  ...
  first_clustering = LocalSearch::ComputeLocalOptimum(
      graph, first_clustering, first_cluster_vertex, second_cluster_vertex);
  ...
  for (unsigned i = 0; i < graph.Size(); ++i) {
    if ((i == second_cluster_vertex ||
         graph.IsJoined(i, second_cluster_vertex)) &&
        i != first_cluster_vertex) {
      second_clustering->SetupLabelForVertex(i, FIRST_CLUSTER);
    } else {
      second_clustering->SetupLabelForVertex(i, SECOND_CLUSTER);
    }
  }
  second_clustering = LocalSearch::ComputeLocalOptimum(
      graph, second_clustering, first_cluster_vertex, second_cluster_vertex);

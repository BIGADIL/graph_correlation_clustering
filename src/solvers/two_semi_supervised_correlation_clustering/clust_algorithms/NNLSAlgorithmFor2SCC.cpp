...
IClustPtr NNLSAlgorithmFor2SCC::getBestNeighborhoodClustering(const IGraph &graph,
  ...
  for (unsigned i = 0; i < num_threads_; i++) {
    thread_vector[i] = std::thread(
        &NNLSAlgorithmFor2SCC::BestNeighborhoodClusteringThreadWorker,
        this,
        std::ref(graph),
        i,
...
void NNLSAlgorithmFor2SCC::BestNeighborhoodClusteringThreadWorker(const IGraph &graph,
  ...
  for (unsigned i = threadId; i < graph.Size(); i += num_threads_) {
    ...
    for (auto &clustering: tmp_neighborhood_clustering) {
      auto ls_clustering = LSAlgorithmFor2SCC::ComputeLocalOptimum(
          graph,
          clustering,
          first_cluster_vertex,
          second_cluster_vertex);
...
NNLSAlgorithmFor2SCC::NNLSAlgorithmFor2SCC(const unsigned num_threads,

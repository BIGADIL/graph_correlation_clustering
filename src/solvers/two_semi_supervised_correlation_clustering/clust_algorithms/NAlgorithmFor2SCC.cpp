...
IClustPtr NAlgorithmFor2SCC::getBestNeighborhoodClustering(const IGraph &graph,
  ...
  for (unsigned i = 0; i < num_threads_; i++) {
    thread_vector[i] = std::thread(
        &NAlgorithmFor2SCC::BestNeighborhoodClusteringThreadWorker,
        this,
        std::ref(graph),
        i,
...
void NAlgorithmFor2SCC::BestNeighborhoodClusteringThreadWorker(const IGraph &graph,
...
NAlgorithmFor2SCC::NAlgorithmFor2SCC(const unsigned num_threads,

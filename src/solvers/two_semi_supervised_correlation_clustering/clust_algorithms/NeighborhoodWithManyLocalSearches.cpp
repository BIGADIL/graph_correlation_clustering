...
IClustPtr semi_supervised_2cc::NeighborhoodWithManyLocalSearches::
    getBestNeighborhoodClustering(const IGraph &graph,
  ...
  for (unsigned i = 0; i < num_threads_; i++) {
    thread_vector[i] = std::thread(&NeighborhoodWithManyLocalSearches::
                                       BestNeighborhoodClusteringThreadWorker,
                                   this, std::ref(graph), i,
...
void semi_supervised_2cc::NeighborhoodWithManyLocalSearches::
    BestNeighborhoodClusteringThreadWorker(
        const IGraph &graph, const unsigned threadId,
        IClustPtr &local_best_clustering, const unsigned first_cluster_vertex,
        const unsigned second_cluster_vertex) const {
  ...
  for (unsigned i = threadId; i < graph.Size(); i += num_threads_) {
    auto tmp_neighborhood_clustering = neighbor_splitter_.SplitGraphByVertex(
        graph, i, first_cluster_vertex, second_cluster_vertex);
    for (auto &clustering : tmp_neighborhood_clustering) {
      const auto ls_clustering = LocalSearch::ComputeLocalOptimum(
          graph, clustering, first_cluster_vertex, second_cluster_vertex);
      if (const unsigned tmp_distance =
              ls_clustering->GetDistanceToGraph(graph);
          tmp_distance < best_distance) {
        best_distance = tmp_distance;
...
semi_supervised_2cc::NeighborhoodWithManyLocalSearches::

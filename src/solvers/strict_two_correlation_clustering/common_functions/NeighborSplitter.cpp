...
strict_2cc::NeighborSplitter::NeighborSplitter(
    IClustFactoryPtr clustering_factory)
    : clustering_factory_(std::move(clustering_factory)) {}

IClustPtr strict_2cc::NeighborSplitter::SplitGraphByVertex(
    const IGraph &graph, const unsigned vertex,
    const unsigned opposite_vertex) const {
  IClustPtr split_clustering =

...
IClustPtr NNLSAlgorithmForS2CC::getBestNeighborhoodClustering(const IGraph &graph) const {
  ...
  for (unsigned i = 0; i < num_threads_; i++) {
    thread_vector[i] = std::thread(
        &NNLSAlgorithmForS2CC::BestNeighborhoodClusteringThreadWorker,
        this,
        std::ref(graph),
        i,
...
void NNLSAlgorithmForS2CC::BestNeighborhoodClusteringThreadWorker(const IGraph &graph,
  ...
  for (unsigned i = threadId; i < graph.Size(); i += num_threads_) {
    for (unsigned j = 0; j < graph.Size(); j++) {
      ...
      tmp_neighborhood_clustering = LSAlgorithmForS2CC::ComputeLocalOptimum(graph, tmp_neighborhood_clustering, i, j);
...
NNLSAlgorithmForS2CC::NNLSAlgorithmForS2CC(const unsigned num_threads,

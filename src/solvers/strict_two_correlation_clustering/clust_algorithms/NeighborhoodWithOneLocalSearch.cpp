...
IClustPtr
strict_2cc::NeighborhoodWithOneLocalSearch::getBestNeighborhoodClustering(
  ...
  for (unsigned i = 0; i < num_threads_; i++) {
    thread_vector[i] = std::thread(
        ...
        this, std::ref(graph), i, std::ref(local_best_clustering_vector[i]),
        std::ref(vertices[i]), std::ref(opposite_vertices[i]));
  }
  ...
  for (unsigned i = 1; i < num_threads_; ++i) {
    if (const auto tmp_distance =
            local_best_clustering_vector[i]->GetDistanceToGraph(graph);
...
void strict_2cc::NeighborhoodWithOneLocalSearch::
    BestNeighborhoodClusteringThreadWorker(const IGraph &graph,
  ...
  for (unsigned i = threadId; i < graph.Size(); i += num_threads_) {
    for (unsigned j = 0; j < graph.Size(); j++) {
      ...
      auto tmp_neighborhood_clustering =
          neighbor_splitter_.SplitGraphByVertex(graph, i, j);
...
strict_2cc::NeighborhoodWithOneLocalSearch::NeighborhoodWithOneLocalSearch(

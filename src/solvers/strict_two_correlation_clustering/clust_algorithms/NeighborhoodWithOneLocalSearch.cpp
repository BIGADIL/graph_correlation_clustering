...
IClustPtr strict_2cc::NeighborhoodWithOneLocalSearch::getBestNeighborhoodClustering(const IGraph& graph) const {
  ...
  for (unsigned i = 0; i < num_threads_; i++) {
    thread_vector[i] =
        std::thread(&NeighborhoodWithOneLocalSearch::BestNeighborhoodClusteringThreadWorker, this, std::ref(graph), i,
                    std::ref(local_best_clustering_vector[i]), std::ref(vertices[i]), std::ref(opposite_vertices[i]));
  }
  ...
  for (unsigned i = 1; i < num_threads_; ++i) {
    if (const auto tmp_distance = local_best_clustering_vector[i]->GetDistanceToGraph(graph);
...
void strict_2cc::NeighborhoodWithOneLocalSearch::BestNeighborhoodClusteringThreadWorker(
    const IGraph& graph, const unsigned threadId, IClustPtr& local_best_clustering, unsigned& vertex,

...
IClustPtr set_semi_supervised_2cc::LocalSearch::ComputeLocalOptimum(
    const IGraph& graph, const IClustPtr& cur_clustering, const std::vector<unsigned>& first_cluster_vertices,
  ...
  while (true) {
    ...
    for (unsigned i = 0; i < graph.Size(); i++) {
      ...
      if (const auto tmp_local_improvement = ComputeLocalImprovement(graph, result, i);
...
int set_semi_supervised_2cc::LocalSearch::ComputeLocalImprovement(const IGraph& graph, const IClustPtr& cur_clustering,

...
IClustPtr set_semi_supervised_2cc::LocalSearch::ComputeLocalOptimum(
  ...
  while (true) {
    ...
    for (unsigned i = 0; i < graph.Size(); i++) {
      if (IsVertexInSet(i, first_cluster_vertices) ||
        ...
        continue;
      if (const auto tmp_local_improvement =
              ComputeLocalImprovement(graph, result, i);
          tmp_local_improvement > local_improvement) {
...
int set_semi_supervised_2cc::LocalSearch::ComputeLocalImprovement(
    const IGraph &graph, const IClustPtr &cur_clustering,
...
bool set_semi_supervised_2cc::LocalSearch::IsVertexInSet(
    const unsigned int vertex, const std::vector<unsigned> &vertices_set) {

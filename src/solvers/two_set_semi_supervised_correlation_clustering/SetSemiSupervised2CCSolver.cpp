...
std::string set_semi_supervised_2cc::SetSemiSupervised2CCSolver::solve(
    const IGraphPtr& graph, const double density, std::vector<std::string> used_algorithms,
  ...
  if (std::ranges::find(used_algorithms, "NeighborhoodWithManyLocalSearches") != used_algorithms.end()) {
    ...
    auto clustering = nmls.getBestNeighborhoodClustering(*graph, first_cluster_vertices, second_cluster_vertices);
  ...
  if (std::ranges::find(used_algorithms, "NeighborhoodWithOneLocalSearch") != used_algorithms.end()) {
    ...
    auto start_time = std::chrono::steady_clock::now();
    auto clustering = nols.getBestNeighborhoodClustering(*graph, first_cluster_vertices, second_cluster_vertices);
    infos.emplace_back("NeighborhoodWithOneLocalSearch", clustering, clustering->GetDistanceToGraph(*graph),
                       std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - start_time));
  }
  if (std::ranges::find(used_algorithms, "NeighborhoodOfPreClusteringVertices") != used_algorithms.end()) {
    NeighborhoodOfPreClusteringVertices npcv(factory_);
    auto start_time = std::chrono::steady_clock::now();
    auto clustering = npcv.getBestNeighborhoodClustering(*graph, first_cluster_vertices, second_cluster_vertices);
    infos.emplace_back("NeighborhoodOfPreClusteringVertices", clustering, clustering->GetDistanceToGraph(*graph),
                       std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - start_time));
  }
  if (std::ranges::find(used_algorithms, "Neighborhood") != used_algorithms.end()) {
    Neighborhood n(num_threads_, factory_);
    auto start_time = std::chrono::steady_clock::now();
    auto clustering = n.getBestNeighborhoodClustering(*graph, first_cluster_vertices, second_cluster_vertices);
  ...
  if (std::ranges::find(used_algorithms, "BranchAndBounds") != used_algorithms.end()) {
    NeighborhoodWithManyLocalSearches nmls(num_threads_, factory_);
    auto approximate_clustering =
        nmls.getBestNeighborhoodClustering(*graph, first_cluster_vertices, second_cluster_vertices);
    BranchAndBounds bb;
    auto start_time = std::chrono::steady_clock::now();
    auto clustering =
        bb.GetBestClustering(graph, approximate_clustering, first_cluster_vertices, second_cluster_vertices);
  ...
  if (std::ranges::find(used_algorithms, "BrutForce") != used_algorithms.end()) {
    ...
    auto start_time = std::chrono::steady_clock::now();
    auto clustering = bf.GetBestClustering(graph, first_cluster_vertices, second_cluster_vertices);
    infos.emplace_back("BrutForce", clustering, clustering->GetDistanceToGraph(*graph),
                       std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - start_time));
  }
  return FormatComputationToJson(*graph, infos, graph->Size(), density, first_cluster_vertices,
...
std::string set_semi_supervised_2cc::SetSemiSupervised2CCSolver::FormatComputationToJson(
    ...
    const double density, const std::vector<unsigned>& first_cluster_vertices,

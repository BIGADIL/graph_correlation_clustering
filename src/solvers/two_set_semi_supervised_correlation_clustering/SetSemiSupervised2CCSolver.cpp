...
std::string set_semi_supervised_2cc::SetSemiSupervised2CCSolver::solve(
  ...
  if (std::ranges::find(used_algorithms, "NeighborhoodWithManyLocalSearches") !=
    ...
    auto clustering = nmls.getBestNeighborhoodClustering(
        *graph, first_cluster_vertices, second_cluster_vertices);
  ...
  if (std::ranges::find(used_algorithms, "BrutForce") !=
    ...
    auto start_time = std::chrono::steady_clock::now();
    auto clustering = bf.GetBestClustering(graph, first_cluster_vertices,
...
set_semi_supervised_2cc::SetSemiSupervised2CCSolver::SetSemiSupervised2CCSolver(
    const unsigned num_threads, IClustFactoryPtr factory)
    : num_threads_(num_threads), factory_(std::move(factory)) {}

std::string
set_semi_supervised_2cc::SetSemiSupervised2CCSolver::FormatComputationToJson(

...
set_semi_supervised_2cc::NeighborhoodOfPreClusteringVertices::
    NeighborhoodOfPreClusteringVertices(IClustFactoryPtr clustering_factory)
    : clustering_factory_(std::move(clustering_factory)) {}

IClustPtr set_semi_supervised_2cc::NeighborhoodOfPreClusteringVertices::
    getBestNeighborhoodClustering(
        const IGraph &graph,
  ...
  for (unsigned i = 0; i < graph.Size(); ++i) {
    if (IsJoinedWithSet(first_cluster_vertices, i, graph) &&
  ...
  first_clustering = LocalSearch::ComputeLocalOptimum(
      graph, first_clustering, first_cluster_vertices, second_cluster_vertices);
  ...
  for (unsigned i = 0; i < graph.Size(); ++i) {
    if (IsJoinedWithSet(second_cluster_vertices, i, graph) &&
  ...
  second_clustering = LocalSearch::ComputeLocalOptimum(graph, second_clustering,
                                                       first_cluster_vertices,
                                                       second_cluster_vertices);
  const auto second_distance = second_clustering->GetDistanceToGraph(graph);
  return first_distance < second_distance ? first_clustering
                                          : second_clustering;
}

bool set_semi_supervised_2cc::NeighborhoodOfPreClusteringVertices::
    IsJoinedWithSet(const std::vector<unsigned> &set, const unsigned vertex,
                    const IGraph &graph) {
...
bool set_semi_supervised_2cc::NeighborhoodOfPreClusteringVertices::
    IsVertexInSet(const unsigned int vertex,

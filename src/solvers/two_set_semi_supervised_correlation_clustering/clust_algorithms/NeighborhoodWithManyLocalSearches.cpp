...
IClustPtr set_semi_supervised_2cc::NeighborhoodWithManyLocalSearches::
...
void set_semi_supervised_2cc::NeighborhoodWithManyLocalSearches::
    BestNeighborhoodClusteringThreadWorker(
  ...
  for (unsigned i = threadId; i < graph.Size(); i += num_threads_) {
    ...
    for (auto &clustering : tmp_neighborhood_clustering) {
      const auto ls_clustering = LocalSearch::ComputeLocalOptimum(
          graph, clustering, first_cluster_vertices, second_cluster_vertices);
...
set_semi_supervised_2cc::NeighborhoodWithManyLocalSearches::
    NeighborhoodWithManyLocalSearches(

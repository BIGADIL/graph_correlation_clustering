  ...
  for (unsigned i = 0; i < num_threads_; i++) {
    thread_vector[i] =
        std::thread(&NeighborhoodWithManyLocalSearches::BestNeighborhoodClusteringThreadWorker, this, std::ref(graph),
                    i, std::ref(local_best_clustering_vector[i]), first_cluster_vertices, second_cluster_vertices);
  }
  ...
  for (unsigned i = threadId; i < graph.Size(); i += num_threads_) {
    ...
    for (auto& clustering : tmp_neighborhood_clustering) {
      const auto ls_clustering =
          LocalSearch::ComputeLocalOptimum(graph, clustering, first_cluster_vertices, second_cluster_vertices);
...
set_semi_supervised_2cc::NeighborhoodWithManyLocalSearches::NeighborhoodWithManyLocalSearches(

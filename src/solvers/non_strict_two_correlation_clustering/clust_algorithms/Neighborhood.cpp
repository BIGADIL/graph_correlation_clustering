...
non_strict_2cc::Neighborhood::Neighborhood(const unsigned num_threads, const IClustFactoryPtr& clustering_factory)
...
std::vector<Solution> non_strict_2cc::Neighborhood::getAllSolutions(const IGraph& graph) const {
  ...
  for (unsigned i = 0; i < num_threads_; i++) {
    thread_vector[i] = std::thread(&Neighborhood::BestNeighborhoodClusteringThreadWorker, this, std::ref(graph), i,

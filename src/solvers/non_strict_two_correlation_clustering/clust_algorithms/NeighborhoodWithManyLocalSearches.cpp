...
IClustPtr non_strict_2cc::NeighborhoodWithManyLocalSearches::
    getBestNeighborhoodClustering(const IGraph &graph) const {
...
void non_strict_2cc::NeighborhoodWithManyLocalSearches::
  ...
  for (unsigned i = threadId; i < graph.Size(); i += num_threads_) {
    auto tmp_neighborhood_clustering =
        neighbor_splitter_.SplitGraphByVertex(graph, i);
    tmp_neighborhood_clustering =
        LocalSearch::ComputeLocalOptimum(graph, tmp_neighborhood_clustering);
    unsigned tmp_distance =
        tmp_neighborhood_clustering->GetDistanceToGraph(graph);
    local_thread_buffer.emplace_back(tmp_distance, tmp_neighborhood_clustering);
  }
}

non_strict_2cc::NeighborhoodWithManyLocalSearches::
...
std::vector<Solution>
non_strict_2cc::NeighborhoodWithManyLocalSearches::getAllSolutions(
  ...
  for (unsigned i = 0; i < num_threads_; i++) {
    thread_vector[i] =
        std::thread(&NeighborhoodWithManyLocalSearches::
                        BestNeighborhoodClusteringThreadWorker,

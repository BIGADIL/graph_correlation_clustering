...
TagsGraphFactory::TagsGraphFactory(const std::string &path, std::string distribution) : distribution_(
...
double TagsGraphFactory::Chance(const std::vector<std::string> &vec1,
    ...
    if (distribution_.find("cosine") != std::string::npos) {
        return static_cast<double>(intersection_set.size()) / sqrt(
    ...
    if (distribution_.find("overlap") != std::string::npos) {
        return static_cast<double>(intersection_set.size()) / std::min(static_cast<double>(set_1.size()),

...
IClustPtr BinaryClusteringVector::GetCopy() const { return std::make_shared<BinaryClusteringVector>(*this); }

ClusterLabels BinaryClusteringVector::GetLabel(const unsigned vertex) const { return labels_[vertex]; }

bool BinaryClusteringVector::IsNonClustered(const unsigned vertex) const { return labels_[vertex] == NON_CLUSTERED; }

bool BinaryClusteringVector::IsSameClustered(const unsigned i, const unsigned j) const {
  return labels_[i] == labels_[j];
}

unsigned BinaryClusteringVector::GetNumNonClusteredVertices() const { return num_non_clustered_vertices_; }
...
unsigned BinaryClusteringVector::Size() const { return labels_.size(); }

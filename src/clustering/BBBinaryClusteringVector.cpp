...
#include "../../include/clustering/BBBinaryClusteringVector.hpp"

BBBinaryClusteringVector::BBBinaryClusteringVector(const unsigned size,
...
void BBBinaryClusteringVector::SetupLabelForVertex(const unsigned vertex,
    ...
    for (unsigned long i = 0; i < labels_.size(); i++) {
        ...
        const auto i_label = labels_[i];
        const auto is_joined = graph_->IsJoined(vertex, i);
...
unsigned BBBinaryClusteringVector::Choose() const {
...
unsigned BBBinaryClusteringVector::Bound(const unsigned record) const {
    auto result = obj_func_value_on_partially_built_clustering_;
    if (result >= record) {
        return result;
    }
    const int u = static_cast<int>(num_non_clustered_vertices_);
    const int delta3 = u * (u - 2) / 4 - num_of_edges_of_non_constructed_graph_;

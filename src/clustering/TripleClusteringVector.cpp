...
void TripleClusteringVector::SetupLabelForVertex(const unsigned vertex, const ClusterLabels label) {
  switch (label) {
    case FIRST_CLUSTER:
    ...
    case SECOND_CLUSTER:
      ...
      break;
    case THIRD_CLUSTER:
      num_vertices_in_third_cluster_++;
      break;
    default:
      const auto message = "Expected label 0 or 1, actual label = " + std::to_string(label);
      throw std::invalid_argument(message);
  }
  switch (const auto cur_label = labels_[vertex]) {
    case NON_CLUSTERED:
...
IClustPtr TripleClusteringVector::GetCopy() const { return std::make_shared<TripleClusteringVector>(*this); }

ClusterLabels TripleClusteringVector::GetLabel(const unsigned vertex) const { return labels_[vertex]; }

bool TripleClusteringVector::IsNonClustered(const unsigned vertex) const { return labels_[vertex] == NON_CLUSTERED; }

bool TripleClusteringVector::IsSameClustered(const unsigned i, const unsigned j) const {
  return labels_[i] == labels_[j];
}

unsigned TripleClusteringVector::GetNumNonClusteredVertices() const { return num_non_clustered_vertices_; }
...
unsigned TripleClusteringVector::Size() const { return labels_.size(); }

...
void TripleClusteringVector::SetupLabelForVertex(const unsigned vertex,
  ...
  switch (label) {
    ...
    default:
      const auto message =
          "Expected label 0 or 1, actual label = " + std::to_string(label);
  ...
  switch (const auto cur_label = labels_[vertex]) {
    ...
    case THIRD_CLUSTER:
      ...
      break;
    default:
      throw std::invalid_argument("undefined cur label=" +
                                  std::to_string(cur_label));
  }
...
unsigned TripleClusteringVector::GetDistanceToGraph(const IGraph &graph) const {
  if (graph.Size() != labels_.size()) {
    const auto message =
        "Graph size must be equal to labels length. Graph size = " +
  ...
  for (unsigned i = 0; i < graph.Size(); i++) {
    for (unsigned j = i + 1; j < graph.Size(); j++) {
      if ((labels_[i] != labels_[j] && graph.IsJoined(i, j)) ||
          (labels_[i] == labels_[j] && !graph.IsJoined(i, j))) {
        distance++;
      }
    }
  }
...
TripleClusteringVector::TripleClusteringVector(const unsigned size) {
  ...
  num_vertices_in_first_cluster_ = num_vertices_in_second_cluster_ =
      num_vertices_in_third_cluster_ = 0;
}
...
unsigned int TripleClusteringVector::GetNumVerticesByLabel(
    const ClusterLabels label) const {
  switch (label) {
    case FIRST_CLUSTER:

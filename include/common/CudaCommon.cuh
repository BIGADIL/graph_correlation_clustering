  ...
  do {                                                                      \
    ...
    inline std::vector<uint8_t> FlattenGraph(const IGraph &graph) {
        ...
        const unsigned num_threads =
            std::clamp<unsigned>(hw == 0 ? 1 : hw, 1, std::max(1u, n / 64));
  ...
  do {                                                                      \
    ...
    if (st__ != CUBLAS_STATUS_SUCCESS) {                                    \
      throw std::runtime_error(std::string("cuBLAS error ") +               \
                               std::to_string(static_cast<int>(st__)) +     \
                               " (" + std::string(__FILE__) + ":" +         \
                               std::to_string(__LINE__) + ")");             \
    }                                                                       \
  } while (0)
     ...
     */
    inline void GemmInt8(const CublasHandle &handle, const unsigned m,
                         const unsigned ncols, const unsigned k,
        ...
        const int alpha = 1;
        const int beta = 0;
        GCC_CUBLAS_CHECK(cublasGemmEx(
            handle.Get(), CUBLAS_OP_T, CUBLAS_OP_N, static_cast<int>(m),
            static_cast<int>(ncols), static_cast<int>(k), &alpha, a, CUDA_R_8I,
            static_cast<int>(k), b, CUDA_R_8I, static_cast<int>(k), &beta, c,
     ...
     */
    __global__ inline void BuildPaddedMatrixKernel(
        const uint8_t *__restrict__ adj, const unsigned n, const unsigned n_pad,
        ...
        if (row < n && col < n) {
            if (row == col) {
                ...
                value = adj[static_cast<size_t>(col) * n + row] != 0
                            ? joined_value
                            : not_joined_value;
            }
        }
    ...
    inline void BuildPaddedMatrix(const uint8_t *adj, const unsigned n,
                                  const unsigned n_pad, const int8_t joined_value,
                                  const int8_t not_joined_value,
                                  const int8_t diagonal_value, int8_t *out) {
        const size_t total = static_cast<size_t>(n_pad) * n_pad;
        const unsigned threads = 256;
        const unsigned blocks = static_cast<unsigned>((total + threads - 1) / threads);
        BuildPaddedMatrixKernel<<<blocks, threads>>>(
            adj, n, n_pad, joined_value, not_joined_value, diagonal_value, out);

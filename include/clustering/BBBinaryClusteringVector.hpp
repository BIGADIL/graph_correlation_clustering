 ...
 *
 * It computes specific only for BB algorithm values which allowed algorithm to
 * work faster.
 */
class BBBinaryClusteringVector final : public BinaryClusteringVector {
  /**
  ...
  /**
   * For each vertex contains current value of objective function, which this
   * vertex will contribute if it lies down in first cluster.
   */
  std::vector<int> obj_func_value_increase_relatively_to_first_cluster_;
  /**
   * For each vertex contains current value of objective function, which this
   * vertex will contribute if it lies down in second cluster.
   */
  std::vector<int> obj_func_value_increase_relatively_to_second_cluster_;
  /**
   * For each vertex contains number of neighbours if non-clustered part of
   * graph.
   */
 ...
 public:
   ...
   */
  [[nodiscard]] unsigned Bound(unsigned record) const;

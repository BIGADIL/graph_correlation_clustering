...
class IClustering {
 public:
   ...
   */
  virtual void SetupLabelForVertex(unsigned vertex, ClusterLabels label) = 0;
   ...
   */
  [[nodiscard]] virtual bool IsSameClustered(unsigned i, unsigned j) const = 0;
...
struct Solution {
  ...
  bool operator==(const Solution &other) const {
    ...
    const auto len = clustering->Size();

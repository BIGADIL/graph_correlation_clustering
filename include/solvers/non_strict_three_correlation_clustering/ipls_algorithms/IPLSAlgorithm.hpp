...
class IPLSAlgorithm {
  ...
  [[nodiscard]] Solution Perturbation(const Solution &solution,
                                      double p_perturbation) const;
  ...
  void TrainWorkerLoop(std::vector<Solution> &local_buffer, unsigned thread_id,
                       unsigned max_capacity,
                       std::vector<Solution> &local_optimum) const;

  void TrainWorker(std::vector<Solution> &local_buffer, unsigned thread_id,
                   unsigned max_capacity,
                   std::vector<Solution> &local_optimum) const;

  void InitPopulationWorker(std::vector<Solution> &local_buffer,
                            unsigned thread_id, unsigned population_size) const;
 ...
 public:
  ...
  IPLSAlgorithm(unsigned iterations, unsigned early_stop_num,

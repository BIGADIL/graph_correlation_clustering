...
class BrutForce {
  static bool IsValidClustering(const std::vector<unsigned>& first_cluster_vertices,
                                const std::vector<unsigned>& second_cluster_vertices, unsigned clustering);
 ...
 public:
  ...
  [[nodiscard]] IClustPtr GetBestClustering(const IGraphPtr& graph, const std::vector<unsigned>& first_cluster_vertices,

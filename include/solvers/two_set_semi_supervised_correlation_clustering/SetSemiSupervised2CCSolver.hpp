...
class SetSemiSupervised2CCSolver {
  const std::vector<std::string> allowed_algorithms{
      "NeighborhoodWithManyLocalSearches", "NeighborhoodWithOneLocalSearch", "Neighborhood", "BranchAndBounds"};
 ...
 private:
  static std::string FormatComputationToJson(const IGraph& graph,
                                             ...
                                             double density, const std::vector<unsigned>& first_cluster_vertices,
                                             const std::vector<unsigned>& second_cluster_vertices);

 public:
  SetSemiSupervised2CCSolver(unsigned num_threads, IClustFactoryPtr factory);

  [[nodiscard]] std::string solve(const IGraphPtr& graph, double density, std::vector<std::string> used_algorithms,

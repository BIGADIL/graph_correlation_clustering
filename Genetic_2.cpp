...
std::vector<IGraphPtr> generate_graphs(TagsGraphFactory& graph_factory, const unsigned n, const unsigned size) {
...
int main() {
  TagsGraphFactory graphs_factory(R"(D:\run\graph_correlation_clustering\non_strict_2cc\Tags_so.json)", "dice_0.25");
    ...
    for (auto population_sizes = {8, 16, 32, 64}; auto& population_size : population_sizes) {
      for (auto tournament_sizes = {5, 10, 15}; auto& tournament_size : tournament_sizes) {
        for (auto mutations = {1e-2, 1e-1, 2e-1, 3e-1, 4e-1}; auto& mutation : mutations) {
          ...
          auto algo_g = non_strict_3cc::IPLSAlgorithm(100, early_stop, factory, population_size, tournament_size,
                                                      mutation, num_threads);
          double sum = 0;
          ...
          std::cout << "early_stop=" << early_stop << ", population_size=" << population_size
                    << ", tournament_size=" << tournament_size << ", mutation=" << mutation << ", average_obj=" << sum
                    << std::endl;
        }
      }
    }
